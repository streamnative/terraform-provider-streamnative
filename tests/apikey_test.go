...
func TestApiKey(t *testing.T) {
	...
	rand := terraform.Output(t, terraformOptions, "rand")

	assert.Equal(t, "tf-apikey-test-key-"+rand, resourceApiKey["name"])
	assert.Equal(t, "sndev/tf-apikey-test-key", resourceApiKey["id"])
	assert.Equal(t, "This is a test api key", resourceApiKey["description"])
	assert.Equal(t, "tf-apikey-test-instance", resourceApiKey["instance_name"])
	assert.Equal(t, "True", resourceApiKey["ready"])
	assert.Equal(t, "tf-apikey-test-service-account", resourceApiKey["service_account_name"])
	...
	assert.Equal(t, "tf-apikey-test-key-"+rand, dataApiKey["name"])
	assert.Equal(t, "sndev/tf-apikey-test-key", dataApiKey["id"])
	assert.Equal(t, "This is a test api key", dataApiKey["description"])
	assert.Equal(t, "tf-apikey-test-instance", dataApiKey["instance_name"])
	assert.Equal(t, "True", dataApiKey["ready"])
	assert.Equal(t, "tf-apikey-test-service-account", dataApiKey["service_account_name"])
